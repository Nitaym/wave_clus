function plot_spikes(handles)
    ...
    cluster_lines = {};
    for i = 0:nclusters
        cluster_lines_y = []; %as a flag to don't make the same vector twice
        if ~ (isempty(class0) && i==0)
            % PLOTS SPIKES OR PROJECTIONS
            class_i = eval(['class' num2str(i)]);
            color = colors(mod(i - 1, maxc) + 1,:)*(i~=0);

            % How many spikes we need?
            sup_spikes = length(class_i);
            max_spikes = min(sup_spikes, par.max_spikes_plot);
            % We'll choose a random set of spikes, below the max_spikes_plot
            permut = randperm(sup_spikes);
            permut = permut(1:max_spikes);

            draw_projections(handles, spikes, inspk, par, color, ls, i, permut, class_i);
        
            if i < 4
                [lines_x, lines_y, av, avup, avdown] = get_class_plot_data(handles, i, spikes, class_i, permut, ls, par);
                clus_ax = eval(['handles.spikes' num2str(i)]); 
                cluster_lines{i+1} = {lines_x, lines_y, av, avup, avdown, clus_ax, color};
                plot_data(clus_ax, lines_x, lines_y, av, avup, avdown, color, i, ls, handles);


                % Get the number of spikes in the cluster
                class_spikes = eval(['class' num2str(i)]);
                plot_title(clus_ax, i, spikes, class_spikes, par);
    ...
    for graph_index = 0:nclusters
        ...
        for data_index = 0:nclusters
            if data_index < 4 && graph_index < 4 && ~isempty(cluster_lines{data_index+1}) && graph_index ~= data_index
                checkbox_name = ['handles.always' num2str(data_index) '_checkbox'];
                if get(eval(checkbox_name),'value') == 1
                    % Plot the lines of the cluster
                    clus_ax = cluster_lines{graph_index + 1}{6};
                    lines_x = cluster_lines{data_index + 1}{1};
                    lines_y = cluster_lines{data_index + 1}{2};
                    av = cluster_lines{data_index + 1}{3};
                    avup = cluster_lines{data_index + 1}{4};
                    avdown = cluster_lines{data_index + 1}{5};
                    color = cluster_lines{data_index + 1}{7};
                    plot_data(clus_ax, lines_x, lines_y, av, avup, avdown, color, i, ls, handles);
                end
            end
        end
...
function draw_projections(handles, spikes, inspk, par, color, spike_size, selected_spikes_indices, cluster_index, class_i)
    % PLOTS SPIKES OR PROJECTIONS
    ...
    if get(handles.spike_shapes_button,'value') == 1 && (get(handles.plot_all_button,'value') == 1) && ~strcmp(par.all_classes_ax,'mean')
        % Draw all spikes in the projections plot
        % Optimizing for speed
        cluster_lines_y = spikes(class_i(selected_spikes_indices),:);
        tmpn = size(cluster_lines_y,1);
        cluster_lines_x = repmat([1:spike_size NaN]',1,tmpn);
        cluster_lines_x = reshape(cluster_lines_x, numel(cluster_lines_x), 1);
        cluster_lines_y = [cluster_lines_y'; repmat(NaN, 1, tmpn)];
        cluster_lines_y = reshape(cluster_lines_y, numel(cluster_lines_y), 1);
        line(cluster_lines_x,cluster_lines_y,'color',color,'Parent',handles.projections,'Visible','off');
        xlim(handles.projections, [1 spike_size])
    elseif get(handles.spike_shapes_button,'value') ==1
        % Draw only spike averages in the projections plot
        av   = mean(spikes(class_i,:));
        plot(handles.projections,1:spike_size,av,'color',color,'linewidth',2);
        xlim(handles.projections,[1 spike_size])
    else
        % Draw only the projections of the spikes
        plot(handles.projections,inspk(class_i,1),inspk(class_i,2),'.','Color',color,'markersize',.5);
        axis(handles.projections,'auto');
    end
end


function [lines_x, lines_y, av, avup, avdown] = get_class_plot_data(handles, i, spikes, class_i, permut, ls, par)
    clus_ax = eval(['handles.spikes' num2str(i)]); 
    xlim(clus_ax,'manual');
    xlim(clus_ax,[1 ls]);
    hold(clus_ax,'on')
    ...
    avdown = av - par.to_plot_std * std(spikes(class_i,:));
    lines_x = [];
    lines_y = [];
                
    if get(handles.plot_all_button,'value') == 1
        % optimizing for speed:
        lines_y = spikes(class_i(permut),:);
        tmpn = size(lines_y,1);
        lines_x = repmat([1:ls NaN]',1,tmpn);
        lines_x = reshape(lines_x, numel(lines_x), 1);
        lines_y = [lines_y'; repmat(NaN, 1, tmpn)];
        lines_y = reshape(lines_y, numel(lines_y), 1);
    end
end

function plot_data(clus_ax, lines_x, lines_y, av, avup, avdown, color, i, ls, handles)
    xlim(clus_ax,'manual');
    xlim(clus_ax,[1 ls]);
    hold(clus_ax,'on')
    
    if get(handles.plot_all_button,'value') == 1
        % This line actually plots all the spikes
        line(lines_x, lines_y, 'color', ...
            color, 'Parent', clus_ax);
        
        if i==0
            line(1:ls,av,'color','c','linewidth',2,'Parent',clus_ax)
            line(1:ls,avup,'color','c','linewidth',0.5,'Parent',clus_ax)
            line(1:ls,avdown,'color','c','linewidth',0.5,'Parent',clus_ax)
        else
            line(1:ls,av,'color','k','linewidth',2,'Parent',clus_ax)
            line(1:ls,avdown,'color',[.4 .4 .4],'linewidth',0.5,'Parent',clus_ax)
